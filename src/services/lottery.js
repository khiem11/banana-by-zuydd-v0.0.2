...
export class LotteryService {
  ...
  async getInfoInvite(user) {
    try {
      const { data } = await user.http.get(
        "get_invite_list?inviteType=1&pageNum=1&pageSize=15"
      );
      if (data.code === 0) {
  ...
  async claimLotteryInvite(user, countClaim) {
    const body = {
      claimLotteryType: 2,
    };
    try {
      ...
      if (data.code === 0) {
        user.log.log(
          colors.green(
            `Claim lottery từ lượt giới thiệu thành công, nhận thêm ${countClaim} lượt harvest`
          )
        );
        return true;
      } else {
  ...
  async handleLottery(user) {
    ...
    const dataInvite = await this.getInfoInvite(user);
    if (dataInvite && dataInvite.claim_lottery_count) {
      await this.claimLotteryInvite(user, dataInvite.claim_lottery_count);
    }
